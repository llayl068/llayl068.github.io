...
function populateListProductChoices(slct2) {
    ...
    var optionArray = check(products);
	...
	for (i = 0; i < optionArray.length; i++) {
		...
		var productClass = ("product productwrapper");
		productDiv.className = productClass;
		productDiv.id = "prod";
		
		s2.appendChild(productDiv);
		...
		label.appendChild(document.createTextNode(product.name.concat(" :",product.price)));
		s2.appendChild(label);
...
function selectedItems(){
	...
	c.className = "cartfont";
	c.innerHTML = "";
	
	var productlist=check(products);
	// build list of selected item
	var para = document.createElement("P");
	para.innerHTML = "You selected : ";
	para.className = "cartfont";
	var price;
	para.appendChild(document.createElement("br"));
	for (i = 0; i < ele.length; i++) { 
		if (ele[i].checked) {
			for (j = 0; j<productlist.length; j++){
				if (productlist[j].name==ele[i].value){
					price=productlist[j].price;
				}
			}
			
			para.appendChild(document.createTextNode(ele[i].value + " - " + price));
			para.appendChild(document.createElement("br"));
